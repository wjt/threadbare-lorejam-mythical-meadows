...
# --- LÓGICA DE INPUT (DISPARO) ---
func _input(event: InputEvent) -> void:
	if Engine.is_editor_hint(): return
	if mode != Mode.USER_CONTROLLED: return # Solo dispara si tiene el control
...
func take_damage(amount: float) -> void:
	...
	update_health_bar()
	if health <= 0:
		defeat(true) # Usa la muerte del framework principal
...
func apply_upgrade(type: String) -> void:
	match type:
		"speed":
			# Modifica el recurso CharacterSpeeds del framework
			if speeds:
