...
# Referencias a los nuevos nodos y al botón
@onready var boton_nuevo: Button = $ContenedorBotones/BotonNuevo
@onready var fondo_negro: ColorRect = $FondoNegro
@onready var texto_transicion: Label = $FondoNegro/TextoTransicion
...
func _ready() -> void:
	# Nos aseguramos de que la transición empiece invisible y oculta al arrancar
	fondo_negro.visible = false
	fondo_negro.modulate.a = 0.0
	...
	# Conectamos el botón mediante código para que ejecute la transición
	boton_nuevo.pressed.connect(_on_continuar_pressed)
...
# Esta función se activa automáticamente al dar clic en BotonNuevo
func _on_continuar_pressed() -> void:
	# Hacemos visible el contenedor (aún no se ve porque su opacidad/alfa es 0)
	fondo_negro.visible = true

	# Creamos el Tween para desvanecer hacia negro e iluminar el texto
	var tween_transicion := create_tween().set_parallel(true)
	tween_transicion.tween_property(fondo_negro, "modulate:a", 1.0, 1.5)  # Tarda 1.5 segundos en aparecer completamente

	# Esperamos a que la animación termine de completarse
	await tween_transicion.finished

	# Dejamos una pequeña pausa dramática de 2 segundos para que lean el texto en la oscuridad
	await get_tree().create_timer(2.0).timeout

	# Cambiamos finalmente a la escena de juego
	SceneSwitcher.change_to_file_with_transition(
		"res://scenes/quests/lore_quests/quest_004/TestEscene.tscn",
		^"",
