...
@export var summon_health_percent: float = 0.5
var summoned_75 := false
var summoned_50 := false
var summoned_25 := false
...
@onready var detect_sound: AudioStreamPlayer2D = get_node_or_null("DetectSound")
@onready var spawn_sound: AudioStreamPlayer2D = get_node_or_null("SpawnSound")
...
@export var repel_force: float = 0.0
...
func _on_detection_area_body_entered(body: Node2D) -> void:
	if body is Player or body.is_in_group("player"):
		...
		if detect_sound:
			detect_sound.stop()
			detect_sound.play()
			print("Boss detectó al jugador y sonó el audio")
		else:
			print("No se encontró DetectSound")
...
func check_summon() -> void:
	var current_percent := float(health) / float(max_health)

	if current_percent <= 0.75 and not summoned_75:
		summoned_75 = true
		summon_enemies()

	if current_percent <= 0.50 and not summoned_50:
		summoned_50 = true
		summon_enemies()

	if current_percent <= 0.25 and not summoned_25:
		summoned_25 = true
		summon_enemies()


func summon_enemies() -> void:
	if summoned_enemy_scene == null:
		print("No asignaste el enemigo luna normal en el inspector")
		return
	...
	for point in spawn_points:
		var enemy: CharacterBody2D = summoned_enemy_scene.instantiate()
		get_tree().current_scene.call_deferred("add_child", enemy)
		enemy.call_deferred("set_global_position", point.global_position)

	print("Boss Luna invocó enemigos")
